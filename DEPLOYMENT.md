...
проверит его по правилам production, создаст активного администратора и
...
**Роли.** Новая установка получает три роли: «Администратор» (все права,
не меняется), «Главный редактор» и «Редактор». Права двух последних
администратор меняет на экране «Пользователи → Роли и права» и там же создаёт
свои роли. Повторный запуск `RolePermissionSeeder` его правки не затирает.
На установке с прежними девятью ролями миграция `consolidate_roles` сливает
суперадминистратора с администратором, а прежние роли, в которых есть
сотрудники, оставляет с теми же правами — администратор переведёт людей на
основные роли и удалит лишние.
...
### 2.12. Оповещения и внешний мониторинг (Telegram)

Два слоя, оба пишут в один чат дежурных.

**1. Критические события CMS.** Упавшее задание планировщика или очереди,
переключение очереди на запасную, сайт перестал принимать вебхук — CMS сама
пишет об этом в Telegram. Одинаковые сообщения — не чаще раза в 10 минут.

1. Создайте бота у [@BotFather](https://t.me/BotFather) и сохраните его токен.
2. Создайте группу (или канал) дежурных и добавьте туда бота.
3. Напишите в группу любое сообщение и откройте
   `https://api.telegram.org/bot<ТОКЕН>/getUpdates` — `chat.id` в ответе и есть
   идентификатор чата (у групп он отрицательный).
4. Пропишите в `.env` CMS и обновите кэш конфигурации:

   ```dotenv
   TELEGRAM_ALERTS_BOT_TOKEN=<токен бота>
   TELEGRAM_ALERTS_CHAT_ID=<идентификатор чата>
   ```

   ```bash
   php artisan config:cache
   php artisan ops:alert-test   # в чате появится пробное сообщение
   ```

**2. Внешняя проверка доступности.** О собственном падении сервер сообщить не
может, поэтому нужен сервис снаружи — например, UptimeRobot или Better Stack:
оба умеют слать оповещения в Telegram. Проверки — раз в 1–5 минут, оповещения —
в тот же чат:

| Адрес | Ожидается | Что значит сбой |
|---|---|---|
| `https://khf.tj/ru` | 200 | публичный сайт недоступен |
| `https://cms.khf.tj/api/v1/health` | 200 | CMS или её база данных недоступны |
| `https://cms.khf.tj/api/v1/ready` | 200 | встали планировщик или очередь (503): не выйдут запланированные публикации, сайт не узнает о правках |

Проверьте цепочку: временно укажите в сервисе несуществующий адрес и
убедитесь, что сообщение пришло.
...
- [ ] Загрузка медиа ограничена типами (изображения + документы, без SVG) и размером: фото до 10 МБ, документы до 20 МБ (`App\Support\UploadLimits`); лимиты PHP и nginx не ниже — §2.10, иначе Центр контроля предупредит.
