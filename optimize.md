...
- [x] JSON-LD и localized 404. **Доказательство:** замер production-сборки по
  18 страницам — `GovernmentOrganization` на всех, `NewsArticle` на всех трёх
  локалях детальной новости, `BreadcrumbList` ровно на тех 11 страницах, где
  есть визуальные крошки (множества файлов с `<Breadcrumbs>` и
  `<BreadcrumbJsonLd>` совпадают полностью). **Что исправлено в разметке:**
  у `NewsArticle` не было `author`/`publisher` (Google сообщает об этом
  предупреждением rich results) и `inLanguage` на трёхъязычном сайте; автор и
  издатель — сам Комитет, персональные данные редактора публичный DTO не
  отдаёт принципиально. Взята короткая форма организации (имя, url, логотип):
  399 B на страницу (204 B gzip) против 815 B (323 B) у полной, при том же
  смысле — полный блок на той же странице уже есть. `dateModified` не добавлен:
  такого поля нет в публичном контракте, а выводить его из времени публикации
  значит соврать о свежести материала. **Что исправлено в 404:**
  `global-not-found` был клиентским и пререндерился один раз, поэтому на любой
  адрес — включая `/tj/…` и `/en/…` — сервер отдавал русскую страницу, а язык
  правила гидратация; теперь это серверный компонент, берущий путь визита из
  заголовка `x-kchs-pathname` от `proxy.ts`: `/tj/nonexistent` → `<html
  lang="tg">` и таджикский текст в самом ответе, `/en/nope` → `en`,
  вложенный `/tj/deep/unknown/path` → `tg`; наибольший маршрут 567,3 →
  562,5 KiB (ушёл клиентский JS страницы 404). Тесты переписаны с «блок есть»
  на проверку пригодности: непустые обязательные поля, абсолютные URL,
  сплошная нумерация крошек, язык блока. Фронт — tsc, ESLint, 112 Vitest,
  production build, `bundle:report` без нарушений, 68 Playwright passed /
  2 skipped.
  **Открытый дефект Next 16 (не наш код):** сырой ответ на несуществующий slug
  уже сматченного маршрута — каркас `<html id="__next_error__">`, то есть без
  JS страница пуста (статус 404 и `noindex` честные). Причина найдена в
  исходниках: при динамическом рендере `notFound()` всегда уходит в
  `getErrorRSCPayload`, а поиск not-found-границы есть только на ветке
  пререндера и требует `experimental.cacheComponents`, который меняет
  семантику кэширования всего приложения. Проверено шестью сборками;
  зафиксировано `test.fixme` с ссылкой на код.
- [x] Security headers и compression. **Доказательство:** контракт из семи
  заголовков проверен на всех видах ответов фронта, а не только на страницах:
  404, `robots.txt`, `sitemap.xml`, хешированная статика. CSP сужен —
  `img-src` больше не разрешает `http:` вне разработки (замер: абсолютных
  адресов картинок в разметке нет, всё идёт через `/_next/image`);
  `script-src 'unsafe-inline'` оставлен осознанно: строгий nonce в Next 16
  требует динамического рендера всех страниц, а их пререндерится 111.
  **Найдено замером и исправлено:** живая CMS не отдавала **ни одного** из
  семи заголовков — ни панель с сессиями редакторов, ни API; добавлен
  middleware `SecurityHeaders`, причём глобальный, потому что на несовпавший
  маршрут middleware групп `web`/`api` не выполняется и 404 всего хоста
  оставался голым. Полный CSP для панели (Inertia + Vite) сознательно
  отложен, взят безопасный сегодня `frame-ancestors 'none'`; HSTS только по
  TLS и без `preload`. **Сжатие:** Next умеет только gzip и не сжимает
  `/sitemap.xml` вовсе (13 185 B вместо 635 B gzip / 509 B brotli); замер
  brotli против gzip — HTML −19,8 %, CSS −9,5 %, JS −3,0 %. Рецепты
  развёртывания приведены в соответствие: у фронтового nginx не было сжатия
  вообще и не было keep-alive к апстриму (HTTP/1.0, новый TCP на запрос) — то
  и другое добавлено вместе с `Accept-Encoding ""`, у CMS-рецепта дописаны
  `application/xml`, заголовки для отдаваемых nginx файлов и
  `expose_php=Off`; в чек-лист после развёртывания добавлены команды
  проверки. CMS — 544 теста / 3984 assertions, PHPStan, Pint; фронт — tsc,
  ESLint, 113 Vitest, production build, 71 Playwright passed / 2 skipped.
  **Попутно найден настоящий дефект медиатеки** (через нестабильный тест):
  поиск использований сравнивал URL вместе с версией `?v=updated_at`, поэтому
  после любой правки картинки её ссылки в текстах переставали находиться — и
  защита «используемый файл нельзя удалить» молча отключалась. Исправлено,
  добавлены детерминированные тесты.
  **Вне пункта:** страницы со `searchParams` (`/ru/news` и другие) отвечают
  `no-store`, то есть CDN их не кэширует — это F-10 «короткий CDN SWR для
  HTML», отдельная работа.
...
- [x] Нормализовать logging/metrics. **Доказательство:** из шести требований
  ...
  71 Playwright.
- [x] Production optimize/OPcache/FPM/CDN. **Доказательство:** проверка
  `ops:production-check` существовала, но сверяла `ini` с конфигом, а сам
  конфиг — ни с чем: 20 000 файлов OPcache были круглым числом на глаз.
  Измерено — **11 944 PHP-файла** в релизе (нужно 14 333 с запасом 20 %), пик
  памяти на запрос **43 MB** при базовой загрузке 36 MB, то есть 95 воркеров
  на 4 GB против консервативных 8 в рецепте. Обе величины перестали быть
  декларацией: `php artisan ops:capacity --ram=<MB>` считает файлы, замеряет
  память на настоящем HTTP-ядре и падает, если OPcache настроен меньше числа
  файлов или воркеры не помещаются в бюджет (4 теста на оба отказа).
  `/health` и `/ready` закрыты 4 тестами на неразглашение: ни APP_KEY, ни
  паролей БД/Redis, ни секрета ревалидации, ни версий PHP/Laravel, ни путей —
  включая путь отказа БД, где драйвер кладёт пароль в текст ошибки, и правило
  CDN «не кэшировать» (`no-store`). **Развёртывание переписано:** было
  `artisan down` + `git pull` в рабочем каталоге (гарантированный простой,
  отката нет вовсе), стало переключение ссылки на готовый каталог релиза с
  `--classmap-authoritative`, `php artisan optimize`, обеими ops-командами как
  гейтом, graceful `reload php-fpm` и `queue:restart`; откат — переключение
  ссылки назад, с явной оговоркой про совместимость миграций, без которой
  мгновенный откат существует только на бумаге. CMS — 558 тестов /
  4064 assertions, PHPStan, Pint.
...
- [x] Queue/scheduler monitoring. **Доказательство:** топология очередей,
  биение воркера, `queue:monitor` по Redis и database-fallback, алерты на
  переполнение/переключение/падение задания и телеметрия длительностей уже
  работали — проверено по `schedule:list` (все восемь заданий с ожидаемой
  периодичностью), а не по коду. **Найденная дыра:** `/ready` следит за
  биением планировщика, но не за успехом его заданий, поэтому падающая ночная
  резервная копия или недельная проверка восстановления не давали ни одного
  сигнала — биение ставит другая команда. Добавлены слушатели
  `ScheduledTaskFailed` (critical) и `ScheduledTaskSkipped` (warning:
  задание, которое раз за разом пропускается по блокировке, стоит на месте) и
  тест на оба события; в runbook добавлен новый повод для алерта. Отметка
  биения намеренно ставится последней строкой команды — это «цикл дошёл до
  конца», а не «команда стартовала». CMS — 559 тестов / 4066 assertions.
- [x] CWV/latency/error dashboards. **Доказательство:** панель существовала
  (точный p75 LCP/INP/CLS по маршрутам и устройствам, p95 API по именованным
  маршрутам, очереди, доля попаданий в кэш), но две её цифры нельзя было
  прочитать. «Ошибки API» складывали 404 от краулера и 500 от упавшего
  сервера — разделены на 5xx и 4xx в сводке и по маршрутам. У p95 не было
  периода: двести замеров могли уложиться в минуту пиковой нагрузки или
  растянуться на трое суток — панель показывает окно выборки. Отдельно
  выяснилось, что браузерный прогон CMS не запускался вовсе: `artisan serve`
  занимал порт, уже проброшенный контейнером; адрес стенда вынесен в
  `CMS_E2E_BASE_URL`, центр контроля добавлен в axe-проверки (serious/critical
  нет). CMS — 560 тестов / 4072 assertions, PHPStan, Pint, tsc, 13/13
  браузерных. **Вне пункта:** тренда во времени нет (нужны временные ряды) и
  ошибки клиентского JS фронта не собираются — записано как известные пробелы.
- [x] Backup + restore drill. **Доказательство:** механика (копия базы и
  оригиналов медиа, манифест с SHA-256, недельная проверка восстановления в
  изолированную схему, тест на порчу файла) существовала — закрыты две дыры.
  **Копии не удалялись никогда:** каждая включает все оригиналы медиа и растёт
  вместе с библиотекой, поэтому диск заполнялся и первым падал следующий
  бэкап. Добавлен `BACKUP_KEEP` (7), обрезка идёт только после успешно
  созданной копии — упавший запуск не уносит вчерашнюю (отдельный тест).
  **Проверялась не та ветка:** тест покрывал SQLite, а production работает на
  MySQL, и его ветка не выполнялась ни разу. Добавлены прогон против
  настоящего MySQL (нога CI; на SQLite пропускается) и прогон с подменёнными
  клиентами, проверяющий аргументы вызова, потоковую запись дампа, совпадение
  контрольной суммы манифеста с файлом и удаление временной схемы. Попутно:
  `mysqldump` часто оказывается клиентом MariaDB, который не умеет
  аутентификацию MySQL 8, — ошибка выглядела как проблема с правами, теперь
  прямо называет причину. CMS — 564 теста / 4086 assertions.
- [x] Media lifecycle/orphan cleanup. **Доказательство:** чистка осиротевших
  производных работала еженедельно — закрыты два конца цикла. `media:audit`
  умел чинить недостающие производные, но **не был в расписании вообще**:
  неудачная конверсия жила, пока её не заметят на сайте; теперь ежедневно с
  `--regenerate`, а когда чинить нечем — падает и поднимает алерт о неудачном
  задании планировщика. Корзина не имела дна: мягкое удаление оставляет
  оригинал и производные на диске навсегда — добавлен `media:purge-trashed` с
  окном `MEDIA_TRASH_GRACE_DAYS` (30) и еженедельным запуском; по умолчанию
  только отчёт, удаление — по `--delete`. Использование проверяется **повторно
  в момент окончательного удаления**: ссылка могла появиться позже, когда
  редактор восстановил старую ревизию. 4 новых теста плюс расширенный тест
  расписания; CMS — 568 тестов / 4105 assertions.
- [x] Load/chaos scenarios. **Доказательство:** шесть хаос-сценариев runbook
  уже были закрыты автоматическими тестами (Redis, вебхук фронта, отсутствие
  `avifenc`, остановка воркера и планировщика, испорченная копия, осиротевший
  файл) — но связи «сценарий → чем проверен» нигде не было, и при правке теста
  список молча стал бы обещанием; теперь это таблица, а тест на runbook
  сторожит наличие имён. **Нагрузку не мерил никто** — добавлен
  `npm run load:probe` (волны одновременных запросов, rps, p50/p95/max,
  статусы, `X-Cache`). Замеры стенда записаны как точка отсчёта: `/home` при 50
  одновременных — ~190–200 rps, p95 250 мс; на только что очищенном кэше модель
  пересобирают 3 запроса из 50 (обвала на холодном старте нет); `/news` без
  read-model-кэша при 200 одновременных — p95 1,0–1,2 с; `/search` под
  нагрузкой отдаёт `429` сверх минутного лимита. CMS — 568 тестов /
  4112 assertions.
- [x] Runbook и rollback. **Доказательство:** документ содержал всё требуемое
  (порядок релиза, откат переключением ссылки, запрет автоматического отката
  миграций, разделы по очереди/Redis/вебхуку/конверсиям/копиям/алертам) — не
  хватало защиты от протухания. Добавлены два стража: каждая упомянутая
  `php artisan …` сверяется с реальным реестром команд, каждый адрес проверки —
  с маршрутами; страж проверен подложенной несуществующей командой. В список
  перед переключением трафика добавлена `ops:capacity`, чтобы о переросшем ini
  узнавать до переключения, а не по симптомам. CMS — 570 тестов /
  4117 assertions.
...
- Lighthouse budgets проходят на контрольных страницах и приложены JSON/HTML-артефакты — **выполнено**: измерение переведено с модели Lantern на реальный троттлинг (у модели LCP этого сайта завышен примерно на секунду: текстовый LCP-элемент она привязывает к загрузке клиентского рантайма), бюджеты выставлены по замеру — FCP 1800 мс, LCP 2500 мс (порог CWV «хорошо»); на контрольных страницах perf 99 / a11y 100;
